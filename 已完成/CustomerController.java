...
public class CustomerController extends HttpServlet {
    ...
    public void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        /** 建立一個新的會員物件 */
        Customer cus = new Customer(name, email, password, birthday, intro);
        ...
        /** 透過MemberHelper物件的checkDuplicate()檢查該會員電子郵件信箱是否有重複 */
        else if (!ch.checkDuplicate(cus)) {
            /** 透過MemberHelper物件的create()方法新建一個會員至資料庫 */
            JSONObject data = ch.create(cus);
            ...
            /** 以字串組出JSON格式之資料 */
            String resp = "{\"status\": \'400\', \"message\": \'註冊帳號失敗，此E-Mail帳號重複！\', \'response\': \'\'}";
            /** 透過JsonReader物件回傳到前端（以字串方式） */
            jsr.response(resp, response);
        }
    }
    ...
    public void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        if(id == -1){
            ...
            int customer_id = ch.login(email, password);

            if (customer_id == -1) {
            	resp.put("status", "401");
                resp.put("message", "登入失敗，查無此帳號");
            } else if(customer_id == -2) {
            	resp.put("status", "401");
                resp.put("message", "登入失敗，密碼錯誤");
            } else {
                resp.put("status", "200");
    ...
    public void doPut(HttpServletRequest request, HttpServletResponse response)
        ...
        /** 取出經解析到JSONObject之Request參數 */
        int id = 6;  //jso.getInt("id"); 登入後id判斷還沒用好
        String name = jso.getString("name");
        String password = jso.getString("password");
        String intro = jso.getString("intro");
        
        /** 透過傳入之參數，新建一個以這些參數之會員Member物件 */
        Customer cus = new Customer(id, name, password, intro);
        
        /** 透過Member物件的update()方法至資料庫更新該名會員資料，回傳之資料為JSONObject物件 */
        JSONObject data = ch.update(cus);
