...
public class CustomerHelper {
    ...
    public JSONObject create(Customer cus) {
        ...
        try {
            ...
            LocalDate nextDay = cus.getBirthday().plusDays(1);
    ...
    public JSONObject update(Customer cus) {
        ...
        try {
            ...
            conn = DBMgr.getConnection();
            /** SQL指令 */
            String sql = "Update `mydb`.`tbl_Customer` SET `customer_name` = ? ,`customer_password` = ? , `modified_time` = ?, `intro` = ? WHERE `customer_id` = ?";
            /** 取得所需之參數 */
