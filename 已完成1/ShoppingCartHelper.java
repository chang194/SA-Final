 ...
 public class ShoppingCartHelper{
            ...
            while(rs.next()) {
                ...
                while(rs_room.next()) {
                    ...
                    while(rs_hotel.next()) {
                        ...
                        sc = new ShoppingCart(room_id,order_number,hotelname,room_price,room_type,image);
    ...
    public JSONObject update(ShoppingCart sc) {
        ...
        try {
            ...
            conn = DBMgr.getConnection();
            /** SQL指令 */
            String sql = "Update `mydb`.`tbl_shoppingcart` SET `order_number` = ?, `checkin_date` = ?, `checkout_date` = ? WHERE `room_id` = ?";
            /** 取得所需之參數 */
            int room_id = sc.getRoomId();
            int order_number = sc.getOrderNumber();
